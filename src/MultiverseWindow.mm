...
void main() {
    ...
    for (int ch = 0; ch < 4; ch++) {
        ...
        if (abs(angle) > 0.001) {
            ...
            sampleUV = rotate2D((sampleUV - 0.5), angleRad) + 0.5;
        }

        // Determine sampling position based on X after rotation
        float xSample = sampleUV.x;

        // Apply curve effect using Y coordinate to offset X sampling
        if (curve > 0.001) {
            // Create a real curve/bend effect, not just skewing
            float yFromCenter = (sampleUV.y - 0.5) * 2.0; // -1 to 1 range

            // Use a parabolic function to create curvature
            // Maximum bend at X=0.5, no bend at X=0 and X=1
            float bendShape = sin(sampleUV.x * 3.14159); // 0 to 1 to 0 curve

            // Increase the strength for more circular effect
            // Curve now goes up to 2.0 for full circle effect
            float bendAmount = yFromCenter * bendShape * curve * 2.0;

            xSample = xSample + bendAmount;

            // Wrap around
            xSample = mod(xSample + 1.0, 1.0);
        }
        ...
        // Increased brightness scaling for better visibility
        float normalizedVoltage = clamp((audioValue + 1.0) * 0.5 * intensity * 0.1, 0.0, 1.0);
    ...
    // Apply overall brightness boost controlled by uniform
    finalColor = finalColor * brightness;
    finalColor = finalColor / (finalColor + vec3(1.0)); // Soft clipping to prevent harsh cutoff
    finalColor = pow(finalColor, vec3(0.8)); // Gamma correction for better visibility
...
@interface MultiverseOpenGLView : NSOpenGLView
{
    ...
    GLint mixModeUniform, timeUniform, brightnessUniform;
}
...
- (void)updateChannelParams:(int)channel
                   ...
                   intensity:(float)i
                   frequency:(float)f;
- (void)updateGlobalParams:(float)mix crossMod:(float)bright;
@end
...
- (void)prepareOpenGL {
    ...
    brightnessUniform = glGetUniformLocation(shaderProgram, "brightness");
