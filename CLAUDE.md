...
### Multiverse 模組（僅限開發資料夾）

**狀態**：不上傳至 GitHub / VCV Library

**原因**：
1. Multiverse 使用 macOS 專用的 Objective-C++ (`MultiverseWindow.mm`)
2. 程式碼使用 `#ifdef ARCH_MAC` 條件編譯，但 `plugin.json` 無法設定條件
3. VCV Library 在 Linux 建置時會報錯：`Manifest contains module Multiverse but it is not defined in plugin`

**檔案位置**（僅存在於開發資料夾）：
- `src/Multiverse.cpp`
- `src/MultiverseWindow.mm`

**同步注意事項**：
- 同步 src 時使用 `rsync --exclude='Multiverse*'` 排除這兩個檔案
- 若未來要支援跨平台，需重構為純 C++ 或使用 VCV 內建的繪圖 API
