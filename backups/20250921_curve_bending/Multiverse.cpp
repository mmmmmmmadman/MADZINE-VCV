...
struct Multiverse : Module {
    ...
    Multiverse() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            configParam(INTENSITY_PARAM_1 + i * 4, 0.f, 1.5f, 1.0f, "Level " + std::to_string(i + 1));
    ...
    void process(const ProcessArgs& args) override {
        ...
        for (int ch = 0; ch < 4; ch++) {
            ...
            // Map 0-1 to -5.699 to 3.301 (0.00001ms to 20 seconds)
            ratio = ratio * 9.0f - 5.699f;
            if (inputs[RATIO_CV_1 + ch * 4].isConnected()) {
                ratio += inputs[RATIO_CV_1 + ch * 4].getVoltage() * 0.9f;
                ratio = clamp(ratio, -5.699f, 3.301f);
            }

            // Calculate samples per pixel based on time ratio
            // Range: 10^-5.699 * 10 = 0.00001ms to 10^3.301 * 10 = 20000ms (20 seconds)
...
struct MultiverseWidget : ModuleWidget {
    ...
    MultiverseWidget(Multiverse* module) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            addParam(createParamCentered<SmallWhiteKnob>(Vec(knobStartX, yPos - 23), module, Multiverse::CURVE_PARAM_1 + i * 4));
        ...
        addInput(createInputCentered<PJ301MPort>(Vec(125, globalControlsY), module, Multiverse::MIX_CV));

        // Global CV input
        addChild(new EnhancedTextLabel(Vec(142, globalControlsY - 21), Vec(30, 10), "GLB", 7.f, nvgRGB(255, 133, 133), true));
        addInput(createInputCentered<PJ301MPort>(Vec(155, globalControlsY), module, Multiverse::GLOBAL_CV));

        addChild(new EnhancedTextLabel(Vec(15, globalControlsY - 21), Vec(30, 10), "TRG", 7.f, nvgRGB(255, 133, 133), true));
        addChild(new EnhancedTextLabel(Vec(70, globalControlsY - 21), Vec(30, 10), "MIX", 7.f, nvgRGB(255, 133, 133), true));
    }
